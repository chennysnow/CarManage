...
namespace SqlTools
{
    ...
    public static partial class SqlableExtensions
    {
        ...
        {
            if (orderBy.Trim() == "") return sqlable;
        ...
        public static List<T> SelectToList<T>(this Sqlable sqlable, string fileds, object whereObj = null, string preSql = null, string nextSql = null) where T : class
        {
            ...
            try
            {
                ...
                var reval = SqlTool.DataReaderToList<T>(typeof(T), sqlable.DB.GetReader(sbSql.ToString(), sqlParams), fileds);
                return reval;
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        private static IDbDataParameter[] GetAllParas(Sqlable sqlable, object whereObj)
        {
            List<IDbDataParameter> allParams = new List<IDbDataParameter>();
            var selectParas = SqlTool.GetParameters(whereObj).ToList();
            ...
            return allParams.ToArray();//.ToArray();
        }
        ...
        public static List<T> SelectToPageList<T>(this Sqlable sqlable, string fileds, string orderByFiled, int pageIndex, int pageSize, object whereObj = null) where T : class
        {
            ...
            try
            {
                ...
                Check.ArgumentNullException(sqlable.Sql, "语法错误，SelectToSql必需要在.Form后面使用");
                SqlTool.GetSqlableSql(sqlable, fileds, orderByFiled, pageIndex, pageSize, sbSql);
                var sqlParams = GetAllParas(sqlable, whereObj);
                var reval = SqlTool.DataReaderToList<T>(typeof(T), sqlable.DB.GetReader(sbSql.ToString(), sqlParams), fileds);
                return reval;
            }
